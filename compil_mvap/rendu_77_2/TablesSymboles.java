...
class TablesSymboles {
    ...
    public void addVarDecl(String name, String t, boolean mutable) {
		if ( _tablesLocales.size() == 0 ) { // On regarde si on est à l’extérieur d’une fonction
			...
			_tableGlobale.addVar(name, VariableInfo.Scope.GLOBAL, t, mutable);
		} else {
			// On a une variable locale 
			_tablesLocales.peek().addVar(name, VariableInfo.Scope.LOCAL, t, mutable);
		}
    }
    ...
    public void addParam(String name, String t, boolean mutable) {
        if ( _tableParam == null ) {
            ...
            _tableParam.addVar(name, VariableInfo.Scope.PARAM, t, mutable);
        }
    }
    ...
    public VariableInfo getReturn() {
	    ...
	    return  new VariableInfo(
                         ...
                         _returnType,
						 true); // haven't done that part yet so I will consider the variable to be mutable
    }
